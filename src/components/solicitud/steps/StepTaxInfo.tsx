...
export default function StepTaxInfo({ applicationId, onNext, onBack }: StepProps) {
    ...
    const handleContinue = async () => {
        ...
        const { data: taxData, error, count } = await supabase
        ...
        if (error) {
            toast.error("Error verificando declaración de renta");
            console.error("Error query tax:", error);
            return;
        }
        ...
        console.log(`Datos de renta encontrados. ${count} registros procesados.`);
        onNext({ taxData });
    };
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="space-y-2">
                <h2 className="text-2xl font-bold text-slate-900 dark:text-white">
                    Información Tributaria 🏛️
                </h2>
                <p className="text-slate-500 dark:text-zinc-400">
                    Sube tu Declaración de Renta (Formulario 110/210). Esperaremos 1 minuto para el análisis.
                </p>
            </div>
            ...
            <div className="max-w-md mx-auto space-y-4">
                <SmartUploader
                    ...
                    label="Declaración de Renta 2023 (PDF)"
                ...
                {isWaiting && (
                    <Card className="bg-amber-50/50 dark:bg-amber-900/10 border-amber-200 dark:border-amber-800 animate-in fade-in zoom-in duration-300">
                        <CardContent className="pt-6">
